...
export function useSessionPersistence(refs: PersistenceRefs) {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        // Re-entrancy guard: if close fires again while we're mid-handle
        // (e.g. the OS re-sends), don't run the save twice.
        let handling = false;
        ...
        const cleanup = getCurrentWindow().onCloseRequested(async (event) => {
        ...
        cleanup
            ...
            .catch((e) => {
                error(`Failed to register close-requested handler: ${e}`).catch(() => {});
            });
        ...
        // Register once; refsRef keeps live state.
